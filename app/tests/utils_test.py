...
class UtilitiesTest(BaseTest):
    def test_checkout(self):
        with self.app_context():
            ...
            self.assertDictEqual(ut.checkout(book, 2), expected,
                                 'Unable to check out a book')

            # Attempt to check out a book that is already checked out
            self.assertTupleEqual(ut.checkout(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to check out a book that is already checked out')
    ...
    def test_checkout_reserved(self):
        with self.app_context():
            ...
            self.assertTupleEqual(ut.checkout(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to check out a book that is reserved')
    ...
    def test_checkout_inactive(self):
        with self.app_context():
            ...
            ut.save_to_db(book)
            self.assertTupleEqual(ut.checkout(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to check out an inactive book')
    ...
    def test_reserve(self):
        with self.app_context():
            ...
            self.assertTupleEqual(ut.reserve(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to reserve a book that is already reserved')
    ...
    def test_reserve_checked_out(self):
        with self.app_context():
            ...
            self.assertTupleEqual(ut.reserve(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to reserve a book that is checked out')
    ...
    def test_reserve_inactive(self):
        with self.app_context():
            ...
            ut.save_to_db(book)
            self.assertTupleEqual(ut.reserve(book, 2), (ut.unavailable_message, 400),
                                  'Did not return error when attempted to reserve an inactive book')
    ...
    def test_release_return(self):
        with self.app_context():
            ...
            expected1 = ({'message': 'book is not checked out'}, 400)
            self.assertTupleEqual(ut.release(book, 1, 'return'), expected1,
                                  'Did not return error when attempting to return a not checked out book')
            ...
            self.assertTupleEqual(ut.release(book, 1, 'return'), expected2,
                                  'Unable to mark book as returned as a book owner')

            # Return a borrowed book
            ut.save_to_db(Lending(book_id=1, user_id=2))
            self.assertTupleEqual(ut.release(book, 2, 'return'), expected2,
            ...
            ut.save_to_db(Lending(book_id=1, user_id=2))
            expected3 = ({'message': 'unauthorized'}, 401)
            self.assertTupleEqual(ut.release(book, 3, 'return'), expected3,
    ...
    def test_release_cancel(self):
        with self.app_context():
            ...
            self.assertTupleEqual(ut.release(book, 1, 'cancel'), expected2,
                                  'Unable to cancel reservation as a book owner')

            # Cancel a previously made reservation
            ut.save_to_db(Reservation(book_id=1, user_id=2))
            self.assertTupleEqual(ut.release(book, 2, 'cancel'), expected2,
            ...
            ut.save_to_db(Reservation(book_id=1, user_id=2))
            expected3 = ({'message': 'unauthorized'}, 401)
            self.assertTupleEqual(ut.release(book, 3, 'cancel'), expected3,
    ...
    def test_print_usage(self):
        with self.app_context():
            lending = Lending(book_id=1, user_id=2)
            ut.save_to_db(lending)
            expected = {
                'book': 'Test Book',
                'user': 'Test Borrower',
                'date_begin': date.today(),
            ...
            self.assertDictEqual(ut.print_usage(lending), expected,
                                 'Returned unexpected information')
